...
public class GerenciadorDeTokenJwt {
	...
	private Claims extrairDetalhesDo(String tokenGerado) {
		return Jwts.parserBuilder().setSigningKey(getChaveDeAssinatura())
				.build()
				.parseClaimsJws(tokenGerado)
				.getBody();
	}
	
	public String extrairLoginDo(String tokeGerado) {
		Claims detalhes = extrairDetalhesDo(tokeGerado);
