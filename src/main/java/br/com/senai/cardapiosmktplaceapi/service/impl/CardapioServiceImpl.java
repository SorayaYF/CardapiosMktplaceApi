...
public class CardapioServiceImpl implements CardapioService{
	...
	private Restaurante getRestaurantePor(NovoCardapio novoCardapio) {
		Preconditions.checkNotNull(novoCardapio.getRestaurante().getId() + "O restaurante é obrigatório");
		Restaurante restaurante = restauranteRepository.buscarPor(
	...
	private Secao getSecaoPor(NovaOpcaoDoCardapio novaOpcao) {
		Preconditions.checkNotNull(novaOpcao.getSecao().getId() + "A seção da opção é obrigatória");
		Secao secao = secoesRepository.findById(novaOpcao.getSecao().getId()).get();
		Preconditions.checkNotNull(secao, "A seção '" + novaOpcao.getSecao().getId() + "' não foi salvo");
	...
	private Opcao getOpcaoPor(Integer idDaOpcao, Restaurante restaurante) {
		...
		Preconditions.checkNotNull(opcao, "Não existe opção vinculado ao id '" + idDaOpcao + "' ");
		Preconditions.checkArgument(opcao.isAtiva(), "A opção '" + opcao.getId() + "' está inativa");
		Preconditions.checkArgument(opcao.getRestaurante().equals(restaurante), "A opção '" + idDaOpcao + "' não pertence ao cardápio do restaurante");
		return opcao;
	}
	...
	private void validarDuplicidadeEm(List<NovaOpcaoDoCardapio> opcoesDoCardapio) {
		for (NovaOpcaoDoCardapio novaOpcao: opcoesDoCardapio) {
			...
			Preconditions.checkArgument(qtdDeOcorrencias == 1, "A opção '" + novaOpcao.getIdDaOpcao() + "' está duplicada no cardápio");
		}
	...
	public void atualizarStatusPor(Integer id, Status status) {
		Cardapio cardapioEncontrado = repository.buscarPor(id);
		Preconditions.checkNotNull(cardapioEncontrado, "Não foi encontrado cardápio para o id informado");
		Preconditions.checkArgument(cardapioEncontrado.getStatus() != status, "O status informado já foi salvo anteriormente");
